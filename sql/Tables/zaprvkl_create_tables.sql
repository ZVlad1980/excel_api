...
--alter table zaprvkl_cross_t drop constraint zaprvkl_cross_sts_chk check (status in ('F', 'P'));
--
create index zaprvkl_cross_hdr_idx on zaprvkl_cross_t(header_id, line_id, status)
-- расскоментить на бою !!! alter table zaprvkl_cross_t add constraint zaprvkl_cross_person_fk foreign key (line_id) references fnd.sp_fiz_lits(ssylka)
/
