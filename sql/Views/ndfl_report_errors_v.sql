create or replace view ndfl_report_errors_v as 
  select dc.data_op,
         ...
         dc.shifr_schet,
         dc.SUB_SHIFR_SCHET,
         dc.summa_source correcting_summa,
  ...
  from   (
          select dc.root_data_op data_op, dc.nom_vkl, dc.nom_ips, dc.shifr_schet, dc.SUB_SHIFR_SCHET, dc.root_ssylka_doc ssylka_doc, dc.det_charge_type,
                 listagg(case when is_leaf = 1 then dc.ssylka_doc || ' (' || to_char(dc.summa_source) || ')' end, ' ,') within group(order by dc.ssylka_doc) corrected_docs,
                 sum(case when is_leaf = 1 then dc.summa_source end) corrected_summa,
                 max(dc.summa_corr) correcting_summa
          from   ndfl_dv_sr_lspv_corr_v dc
