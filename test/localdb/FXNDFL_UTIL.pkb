...
Select ad.SSYLKA,
          ...
          (Select NAZV from GNI_KLADR@Fnd_Fondb where KLCODE=REG_KOD||'00000000000') REG_NAM,
          (Select SOCR from GNI_KLADR@Fnd_Fondb where KLCODE=REG_KOD||'00000000000') REG_SKR,
...
Select 
          ...
          (Select NAZV||' '||SOCR from GNI_KLADR@Fnd_Fondb where KLCODE=substr(PUN_GNK,1,5)||'00000000') RON,
          (Select NAZV||' '||SOCR from GNI_KLADR@Fnd_Fondb where KLCODE=substr(PUN_GNK,1,8)||'00000') GOR,
          (Select NAZV||' '||SOCR from GNI_KLADR@Fnd_Fondb where KLCODE=PUN_GNK) PUN,
...
begin
    ...
    CheckGlobals;
        ...
        Select ls.KOD_NA, ls.GOD, ls.SSYLKA, ls.TIP_DOX, ls.NOM_KORR, ga.TIP_ADR, ga.ADR_FND, 
               ...
               (Select NAZV||' '||SOCR from GNI_KLADR@Fnd_Fondb@Fnd_Fondb where KLCODE=substr(ga.PUN_GNK,1,5)||'00000000' and ga.PUN_GNK not like '__000%'   ) F2_RAYON, 
               (Select NAZV||' '||SOCR from GNI_KLADR@Fnd_Fondb@Fnd_Fondb where KLCODE=substr(ga.PUN_GNK,1,8)||'00000'    and ga.PUN_GNK not like '_____000%') F2_GOROD, 
               (Select NAZV||' '||SOCR from GNI_KLADR@Fnd_Fondb@Fnd_Fondb where KLCODE=ga.PUN_GNK and KLCODE not like '%000__')  F2_PUNKT,
               ga.ULI_NAM||' '||ga.ULI_SKR F2_ULITSA, 
               ga.TXT_DOM F2_DOM, ga.TXT_KOR F2_KOR, ga.TXT_KV F2_KV, ls.NOM_SPR
        from GNI_ADR_SOOTV@Fnd_Fondb ga 
             inner join f2NDFL_LOAD_SPRAVKI ls on ls.SSYLKA=ga.SSYLKA and ls.TIP_DOX=ga.TIP_DOX and ls.NOM_SPR=ga.SPRNOM
        ...
        Select ls.KOD_NA, ls.GOD, ls.SSYLKA, ls.TIP_DOX, ls.NOM_KORR, ga.TIP_ADR, ga.ADR_FND, 
               ...
               (Select NAZV||' '||SOCR from GNI_KLADR@Fnd_Fondb@Fnd_Fondb where KLCODE=substr(ga.ULI_GNK,1, 5)||'00000000' and ga.ULI_GNK not like '__000%'   )     F2_RAYON, 
               (Select NAZV||' '||SOCR from GNI_KLADR@Fnd_Fondb@Fnd_Fondb where KLCODE=substr(ga.ULI_GNK,1, 8)||'00000'    and ga.ULI_GNK not like '_____000%')     F2_GOROD, 
               (Select NAZV||' '||SOCR from GNI_KLADR@Fnd_Fondb@Fnd_Fondb where KLCODE=substr(ga.ULI_GNK,1,11)||'00'       and ga.ULI_GNK not like '________000%')  F2_PUNKT,
               (Select NAZV||' '||SOCR from GNI_STREET@Fnd_Fondb where KLCODE=ga.ULI_GNK and ga.ULI_GNK not like '%0000__') F2_ULITSA, 
               ga.TXT_DOM F2_DOM, ga.TXT_KOR F2_KOR, ga.TXT_KV F2_KV, ls.NOM_SPR
        from GNI_ADR_SOOTV ga 
             inner join f2NDFL_LOAD_SPRAVKI ls on ls.SSYLKA=ga.SSYLKA and ls.TIP_DOX=ga.TIP_DOX and ls.NOM_SPR=ga.SPRNOM
...
begin
        ...
        Select ls.R_SPRID, trim(to_char(ga.STR_KOD,'000')) F2_KODSTR, ga.ADR_FND, ga.PINDEX, substr(ga.PUN_GNK,1,2) F2_KODREG,
               (Select NAZV||' '||SOCR from GNI_KLADR@Fnd_Fondb where KLCODE=substr(ga.PUN_GNK,1,5)||'00000000' and ga.PUN_GNK not like '__000%'   ) F2_RAYON, 
               (Select NAZV||' '||SOCR from GNI_KLADR@Fnd_Fondb where KLCODE=substr(ga.PUN_GNK,1,8)||'00000'    and ga.PUN_GNK not like '_____000%') F2_GOROD, 
               (Select NAZV||' '||SOCR from GNI_KLADR@Fnd_Fondb where KLCODE=ga.PUN_GNK and KLCODE not like '%000__')  F2_PUNKT,
        ...
        Insert into fnd.f2NDFL_ARH_ADR( R_SPRID, KOD_STR, ADR_INO,  PINDEX, KOD_REG, RAYON, GOROD, PUNKT, ULITSA,  DOM, KOR, KV ) 
        Select ls.R_SPRID, trim(to_char(ga.STR_KOD,'000')) F2_KODSTR, ga.ADR_FND, ga.PINDEX, substr(ga.PUN_GNK,1,2) F2_KODREG,
               (Select NAZV||' '||SOCR from GNI_KLADR@Fnd_Fondb where KLCODE=substr(ga.ULI_GNK,1, 5)||'00000000' and ga.ULI_GNK not like '__000%'   )     F2_RAYON, 
               (Select NAZV||' '||SOCR from GNI_KLADR@Fnd_Fondb where KLCODE=substr(ga.ULI_GNK,1, 8)||'00000'    and ga.ULI_GNK not like '_____000%')     F2_GOROD, 
               (Select NAZV||' '||SOCR from GNI_KLADR@Fnd_Fondb where KLCODE=substr(ga.ULI_GNK,1,11)||'00'       and ga.ULI_GNK not like '________000%')  F2_PUNKT,
